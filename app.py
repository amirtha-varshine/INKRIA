...
def generate_notes():
    ...
    try:
        ...
        if include_mindmap:
            level = data.get('mindmap_level', 'precise').strip()
            prompt += (
                '\n\nAdd a section titled "## MIND MAP" formatted as a nested Markdown outline.\n'
                'RULES:\n'
                '1. Use # for the central topic, ## for main branches, ### and #### for sub-branches.\n'
                '2. Do NOT use bullet lists inside the mind map section — use heading levels only.\n'
                '3. No emojis, no icons, no Mermaid syntax.\n'
            )
            if level == 'precise':
                prompt += '4. Keep it CRISP: 5-6 main branches (##), each with 2-3 sub-branches (###). 1-4 words per heading.\n'
            else:
                prompt += '4. Keep it DETAILED: 6-8 main branches (##), each going 3-4 levels deep (###, ####). 3-7 words per heading.\n'
...
def generate_mindmap():
    ...
    try:
        ...
        level = request.form.get('level', 'precise').strip()
        prompt = (
            'Create a highly detailed, hierarchical mind map as a nested Markdown outline.\n'
            'RULES:\n'
            '1. Use # for the central topic, ## for main branches, ### and #### for sub-branches to show hierarchy.\n'
            '2. Do NOT use bullet lists — use heading levels only.\n'
            '3. No emojis, no icons, no Mermaid syntax.\n'
        )
        if level == 'precise':
            prompt += '4. Keep it CRISP: 5-6 main branches (##), each with 2-3 sub-branches (###). 1-4 words per heading.\n'
            ...
            prompt += '4. Keep it DETAILED: 6-8 main branches (##), each going 3-4 levels deep (###, ####). 3-7 words per heading.\n'
        prompt += f'5. Do NOT wrap the result in a markdown code block.\n\nContent/Topic:\n{content[:25000]}'
        result = model.generate_content(prompt).text
